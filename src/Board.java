...
public class Board extends Observable implements Observer{
	...
	{		
		...
		openedGoalCards = new int[3];
		openedGoalCards[TOP_GOAL] = 0;
		openedGoalCards[MIDDLE_GOAL] = 0;
		openedGoalCards[BOTTOM_GOAL] = 0;
		...
		try{
			...
			while(playableCells.size() > 3)			
				playableCells.remove(3);				
			while(filledCells.size() > 0)
				filledCells.remove(0);
	...
	private void updateCells(CardPath card, int x, int y) 
	{
		...
		{		
			...
			try
			{
				...
				if(x == 0)
					if(y==0){
						cells[x][y].updateStates(Cell.EMPTY_NEIGHBOR, cells[x+1][y].getLeftState(), cells[x][y+1].getTopState(), Cell.EMPTY_NEIGHBOR);
					}else if(y==HEIGHT-1){
						cells[x][y].updateStates(cells[x][y-1].getBottomState(), cells[x+1][y].getLeftState(), Cell.EMPTY_NEIGHBOR, Cell.EMPTY_NEIGHBOR);
					}else{
						cells[x][y].updateStates(cells[x][y-1].getBottomState(), cells[x+1][y].getLeftState(), cells[x][y+1].getTopState(), Cell.EMPTY_NEIGHBOR);
					}
				else if(x == WIDTH-1)				
					cells[x][y].updateStates(cells[x][y-1].getBottomState(), Cell.EMPTY_NEIGHBOR, cells[x][y+1].getTopState(), cells[x-1][y].getRightState());
				else if(y == 0)
					cells[x][y].updateStates(Cell.EMPTY_NEIGHBOR, cells[x+1][y].getLeftState(), cells[x][y+1].getTopState(), cells[x-1][y].getRightState());
				else if(y == HEIGHT-1)
					cells[x][y].updateStates(cells[x][y-1].getBottomState(), cells[x+1][y].getLeftState(), Cell.EMPTY_NEIGHBOR, cells[x-1][y].getRightState());
				else
			...
			try
			{
				...
				if(x > 0)
				{
					...
					else if (cells[x-1][y].getRightState() == CardPath.PATH && (int)cardProperties[5] == CardPath.PATH )
					{
						cells[x-1][y].updateStates(cells[x-1][y].getTopState(), Cell.CONNECTED_NEIGHBOR, 
								cells[x-1][y].getBottomState(), cells[x-1][y].getLeftState());
						cells[x][y].updateStates(cells[x][y].getTopState(), cells[x][y].getRightState(), 
								cells[x][y].getBottomState(), Cell.CONNECTED_NEIGHBOR);
					}
				}
				...
				if(y > 0)
				{
					...
					else if (cells[x][y-1].getBottomState() == CardPath.PATH && (int)cardProperties[2] == CardPath.PATH )
					{
						...
						cells[x][y].updateStates(Cell.CONNECTED_NEIGHBOR, cells[x][y].getRightState(), 
								cells[x][y].getBottomState(), cells[x][y].getLeftState());
					}
				}
				...
				if(y < HEIGHT-1)
				{
					...
					else if (cells[x][y+1].getTopState() == CardPath.PATH && (int)cardProperties[4] == CardPath.PATH )
					{
						cells[x][y+1].updateStates(Cell.CONNECTED_NEIGHBOR, cells[x][y+1].getRightState(), 
								cells[x][y+1].getBottomState(), cells[x][y+1].getLeftState());
						cells[x][y].updateStates(cells[x][y].getTopState(), cells[x][y].getRightState(), 
								Cell.CONNECTED_NEIGHBOR, cells[x][y].getLeftState());
					}
				}
				...
				if(x < WIDTH - 1)
				{
					...
					{
						cells[x+1][y].updateStates(cells[x+1][y].getTopState(), cells[x+1][y].getRightState(), cells[x+1][y].getBottomState(), Cell.CONNECTED_NEIGHBOR);
						cells[x][y].updateStates(cells[x][y].getTopState(), Cell.CONNECTED_NEIGHBOR, cells[x][y].getBottomState(), cells[x][y].getLeftState());
					}
				}
			}
			catch(Exception ex){
	...
	{		
		...
		{			
			...
			/*System.out.println(currentCell.getX() + ", " + currentCell.getY() + " STACK: " + toBeChecked.size());
			System.out.println(currentCell.getTopState() + " - " + currentCell.getRightState() + " - " + currentCell.getBottomState() + " - " + currentCell.getLeftState());
			*/
			
			//check four neighboring cells of current cell
			if(currentCell.getLeftState() == Cell.CONNECTED_NEIGHBOR && !isChecked[currentCell.getX()-1][currentCell.getY()])
				toBeChecked.push(cells[currentCell.getX()-1][currentCell.getY()]);
			if(currentCell.getRightState() == Cell.CONNECTED_NEIGHBOR && !isChecked[currentCell.getX()+1][currentCell.getY()])
				toBeChecked.push(cells[currentCell.getX()+1][currentCell.getY()]);		
			if(currentCell.getBottomState() == Cell.CONNECTED_NEIGHBOR && !isChecked[currentCell.getX()][currentCell.getY()+1])				
				toBeChecked.push(cells[currentCell.getX()][currentCell.getY()+1]);
			if(currentCell.getTopState() == Cell.CONNECTED_NEIGHBOR && !isChecked[currentCell.getX()][currentCell.getY()-1])
				toBeChecked.push(cells[currentCell.getX()][currentCell.getY()-1]);																	
		}
		...
		{					
			...
			if(isGoalReached[TOP_GOAL] && !cells[WIDTH-1][0].getCard().isOpened())				
			{
				if(checkConnectedPath(cells[WIDTH-1][0]))				
					...
					if(cells[WIDTH-1][0].getCard().isGoldGoalCard()){
						openedGoalCards[TOP_GOAL] = 1;
					}else{
						openedGoalCards[TOP_GOAL] = -11;
					}
			...
			else if(isGoalReached[MIDDLE_GOAL] && !cells[WIDTH-1][2].getCard().isOpened())
			{
				if(checkConnectedPath(cells[WIDTH-1][2]))				
					...
					if(cells[WIDTH-1][2].getCard().isGoldGoalCard()){
						openedGoalCards[MIDDLE_GOAL] = 1;
					}else{
						openedGoalCards[MIDDLE_GOAL] = -1;
					}
			}
			else if(isGoalReached[BOTTOM_GOAL] && !cells[WIDTH-1][4].getCard().isOpened())
			{
				...
				if(cells[WIDTH-1][4].getCard().isGoldGoalCard()){
					openedGoalCards[BOTTOM_GOAL] = 1;
				}else{
					openedGoalCards[BOTTOM_GOAL] = -1;
				}
			}
			
//			for (int i = 0; i < getFilledCells().size(); i++) {
//				System.out.println("Filled Cells : "+"("+getFilledCells().get(i).getX()+","+getFilledCells().get(i).getY()+")"+" [ "+getFilledCells().get(i).getTopState()+
//						" , "+
//						getFilledCells().get(i).getRightState()+
//						" , "+
//						getFilledCells().get(i).getBottomState()+
//						" , "+
//						getFilledCells().get(i).getLeftState()+" ]\n");
//			}
//			for (int i = 0; i < getPlayableCells().size(); i++) {
//				System.out.println("Playable Cells : "+"("+getPlayableCells().get(i).getX()+","+getPlayableCells().get(i).getY()+")"+" [ "+getPlayableCells().get(i).getTopState()+
//						" , "+
//						getPlayableCells().get(i).getRightState()+
//						" , "+
//						getPlayableCells().get(i).getBottomState()+
//						" , "+
//						getPlayableCells().get(i).getLeftState()+" ]\n");
//			}
		}
	}
	
	public int[] getOpenedGoalCards() {
		return openedGoalCards;
	}

	public void setOpenedGoalCards(int[] openedGoalCards) {
		this.openedGoalCards = openedGoalCards;
	}
